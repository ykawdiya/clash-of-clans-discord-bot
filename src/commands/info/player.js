        ...
        try {
            ...
            if (!clanTag && !clanName) {
                ...
                try {
                    ...
                    return interaction.editReply(`Error retrieving clan data: ${error.message}`);
                }
            ...
            if (clanTag) {
                ...
                let clanData;
                try {
                    clanData = await clashApiService.getClan(formattedTag);
                } catch (error) {
                    return interaction.editReply(`Error retrieving clan data: ${error.message}`);
                }
                return sendClanEmbed(interaction, clanData);
            ...
            if (clanName) {
                ...
                if (searchResults.items.length === 1) {
                    let clanData;
                    try {
                        clanData = await clashApiService.getClan(searchResults.items[0].tag);
                    } catch (error) {
                        return interaction.editReply(`Error retrieving clan data: ${error.message}`);
                    }
                    return sendClanEmbed(interaction, clanData);
...
async function sendClanEmbed(interaction, clanData) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(clanData.description?.trim() ? clanData.description : 'No description')
        ...
        .addFields(
            ...
            { name: 'War League', value: clanData.warLeague?.name || 'Not Assigned', inline: true },
            ...
            { name: 'War Win Streak', value: (clanData.warWinStreak ?? '0').toString(), inline: true },
            { name: 'War Wins', value: (clanData.warWins ?? '0').toString(), inline: true },
